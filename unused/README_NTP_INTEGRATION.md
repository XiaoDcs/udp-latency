...
> ⚠️ 本文档为历史版本，已归档到 `unused/README_NTP_INTEGRATION.md`；最新请优先阅读 `README_DETAIL.md`。
...
- ✅ **自动时间同步**: 固定角色分配：`sender`=NTP服务器，`receiver`=NTP客户端（不再基于IP大小自动判断）
- ✅ **无需地面站**: 两台无人机自主完成时间同步
...
无人机A (192.168.104.10)          无人机B (192.168.104.20)
    ↓                                    ↓
以 sender 模式启动              ←→    以 receiver 模式启动
NTP服务器 (chrony)              ←→    NTP客户端 (chrony)
    ↓                                    ↓
启动GPS记录器                  ←→    启动GPS记录器
    ↓                                    ↓
...
### 角色分配
- **固定分配**：`sender` 作为 NTP 服务器（server），`receiver` 作为 NTP 客户端（client）
- **与IP大小无关**：两端谁是 server/client 由你启动时选择的 `--mode` 决定，而不是 `192.168.104.10 < 192.168.104.20`
- **与 `--ntp-peer-ip` 的关系**：它只影响“对时连接到哪个IP/放行哪个网段”，不会改变 server/client 角色（见参数章节）
